...
export default {
  ...
  methods: {
    ...
    enter(el, done){
      ...
      //小球动画优化思路
      //1.先分析导致动画不准确的本质原因:我们把小球最终位移到的位置，已经局限在了某一分辨率下的滚动条末滚动的情况下；
      //2.只要分辨率和测试的时候不一样，或者滚动条有一定的滚动距离之后问题就出现了；
      //3.因此我们经过分析得到结论:不能把位置的横纵坐标直接写死了，而是应该根据不同情况，动态计算这个坐标值；
      //4.经过分析，得出解题思路:先得到徽标的横纵坐标，再得到小球的横纵坐标，然后y值求差，x值也求差，得到的结果，就是横纵坐标要位移的距离
